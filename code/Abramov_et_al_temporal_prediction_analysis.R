# ---- Predicting interactions across time with SVD ----
...
# function to extract year number from layer name
extract_year <- function(layer_name) {
  # Extracts the numeric part from layer_1, layer_2, etc.
  as.numeric(gsub("layer_", "", layer_name))
}
...
make_full_correlation_plot <- function(data,
                                       evaluator = "f05_score",
                                       parasite_x = "avg_sorensen_parasites",
                                       host_x = "avg_sorensen_hosts",
  ...
  host_plot <- make_simple_correlation_plot(
    data = data,
    x_var = host_x,
    evaluator = evaluator,
    x_lab = NULL,  # No individual x-label
  ...
  host_plot <- host_plot + theme(plot.margin = ggplot2::margin(4, 10, 4, 10))
  parasite_plot <- parasite_plot + theme(plot.margin = ggplot2::margin(4, 10, 4, 10))
  
  
  # Arrange plots side by side
  # plots_side_by_side <- arrangeGrob(
  #   host_plot, parasite_plot,
  #   ncol = 2
  # )
...
make_cor_plot <- function(data, evaluator, 
  ...
  # Compute correlation between evaluator and time difference
...
combine_two_plots <- function(p1, p2,
                              x_axis_label = "Time difference (years)",
                              y_axis_label = "F0.5 score",
                              p1_title = "Early period",
                              p2_title = "Later period",
...
### ---- load matrices ----
# load and mold data
# aggregated_df_old <- load_and_mold_data_for_prediction(emln_id)

# load data/41559_2017_B2Fs415590170101_MOESM36_ESM.xlsx
hp_network <- readxl::read_excel("data/41559_2017_BFs415590170101_MOESM36_ESM.xlsx")

# Aggregate individual hosts to species level by summing parasite counts
hp_network <- hp_network %>%
  group_by(YearCollected, Host) %>%
  summarise(across(where(is.numeric), sum), .groups = "drop")

aggregated_df <- hp_network %>%
  # Create layer identifier from YearCollected
  mutate(layer_from = paste0("layer_", YearCollected),
         layer_to = paste0("layer_", YearCollected)) %>%
  # Convert to long format: pivot all columns except YearCollected and Host
  pivot_longer(
    cols = -c(YearCollected, Host, layer_from, layer_to),
    names_to = "node_to",
    values_to = "weight"
  ) %>%
  # Rename Host to node_from
  rename(node_from = Host) %>%
  # Filter out zero weights (non-interactions)
  filter(weight > 0) %>%
  # Add interaction type
  mutate(type = "host-parasite") %>%
  # Select and reorder columns to match the original aggregated_df structure
  select(layer_from, node_from, layer_to, node_to, weight, type)

# Rename layers to sequential numbers
# Get unique years and create mapping
...
aggregated_df <- aggregated_df %>%
  mutate(
    layer_from = year_mapping[layer_from],
    layer_to = year_mapping[layer_to]
  )

# Store the year mapping for later use (reverse mapping: layer_1 -> 1982, etc.)
layer_to_year <- setNames(unique_years, paste0("layer_", seq_along(unique_years)))

# save aggregated network to a file
# write.csv(aggregated_df, file = "prediction_pipeline_for_publication/results/hp_analysis/network_year_scale.csv", row.names = FALSE)
...
if (file.exists(results_file)) {
  ...
  # combined_results includes predictions for all combinations of years, 50 iterations of links withholding and prediction for each combination
  # save the results
...
host_species <- unique(df$node_from)           # get unique names
parasite_species <- unique(df$node_to)
length(host_species)
...
# 3) average across layer combos and pivot long
df_avg <- df_thresh %>%
  group_by(threshold) %>%
  summarise(across(
...
for (layers_to_train in 1:num_layers) {
  for (layer_to_predict in 1:num_layers) {
    ...
    intersection_parasite <- length(intersect(parasite_train, parasite_test))
    union_parasite        <- length(union(parasite_train, parasite_test))
    jaccard_parasite <- if (union_parasite == 0) NA else intersection_parasite / union_parasite
    
    # 2) Jaccard hosts
    hosts_train <- colnames(A)[ colSums(A) > 0 ]
    hosts_test  <- colnames(P)[ colSums(P) > 0 ]
    intersection_hosts <- length(intersect(hosts_train, hosts_test))
    union_hosts        <- length(union(hosts_train, hosts_test))
    jaccard_hosts <- if (union_hosts == 0) NA else intersection_hosts / union_hosts
...
jaccard_year_f05 <- make_facet_scatter_plot(data = network_results_jaccard, 
                                          evaluator = "f05_score",
                                          pivot_cols = c("jaccard_parasites", "jaccard_hosts", "jaccard_edges"),
                                          x_lab = "Jaccard similarity",
                                          y_lab = "F0.5 score",
                                          facet_scales = "free_x")
jaccard_year_f05 # Fig. 4

# # Base‐R PDF device
# pdf(
#   file   = "jaccard_year_f05.pdf",
#   width  = 7,    # inches
#   height = 3.5,
#   family = "Helvetica"   # or another installed font
# )
# print(jaccard_year_f05)
# dev.off()     # close the file

jaccard_year_nnse <- make_facet_scatter_plot(data = network_results_jaccard, 
                                            evaluator = "nnse",
                                            pivot_cols = c("jaccard_parasites", "jaccard_hosts", "jaccard_edges"),
                                            x_lab = "Jaccard similarity",
                                            y_lab = "NNSE",
...
# calculate degree for each host species (node_from)
host_degree <- df_filtered %>%
  group_by(train_layer, test_layer, node_from) %>%
  summarise(host_degree = n(), .groups = "drop")

# average host degree by train_layer and test_layer
avg_host_degree <- host_degree %>%
  group_by(node_from) %>%
  summarise(avg_host_degree = mean(host_degree), .groups = "drop")

overall_host_degree <- df_filtered %>% 
  group_by(node_from) %>% 
  summarise(overall_host_degree = length(unique(node_to)), .groups = "drop")

# calculate degree for each parasite species (node_to)
parasite_degree <- df_filtered %>%
  group_by(train_layer, test_layer, node_to) %>%
  summarise(parasite_degree = n(), .groups = "drop")

# average parasite degree by train_layer and test_layer
avg_parasite_degree <- parasite_degree %>%
  group_by(node_to) %>%
  summarise(avg_parasite_degree = mean(parasite_degree), .groups = "drop")
...
# for each year pair and interaction, determine if the interaction was observed.
...
df_year_pair <- df %>%
  ...
  summarise(
    ...
    # For predicted_prob_sigm, you might take the average across iterations per year pair.
    year_pair_sigm_predicted = mean(predicted_prob_sigm, na.rm = TRUE),
    .groups = "drop"
  )

table(df_year_pair$observed)

# now, for each unique interaction, compute:
# - The proportion of year pairs where it was observed.
# - The average predicted probability (averaged over year pairs).
df_summary <- df_year_pair %>%
  group_by(node_from, node_to) %>%
  summarise(
    avg_prop = mean(observed, na.rm = TRUE),       # proportion of year pairs with observation
    avg_sigm_predicted = mean(year_pair_sigm_predicted, na.rm = TRUE),
    n_year_pairs = n(),  # number of year pairs contributing
    .groups = "drop"
  )
...
df_never_observed <- df_summary %>%
  ...
  group_by(node_from) %>%
  summarise(count_never_observed = n(), .groups = "drop") #hosts

df_never_observed_parasite <- df_summary %>%
  filter(avg_prop == 0, avg_sigm_predicted > best_discrete_threshold) %>%
  group_by(node_to) %>%
  summarise(count_never_observed = n(), .groups = "drop")

never_hosts_degree <- df_never_observed %>% left_join(avg_host_degree, by="node_from")

# for relating to overall degree

never_hosts_degree_overall <- df_never_observed %>% left_join(overall_host_degree, by="node_from")

never_parasite_degree_overall <- df_never_observed_parasite %>% left_join(overall_parasite_degree, by="node_to")

# calculate correlation
df_to_correlate <- never_hosts_degree_overall
df_to_correlate$x <- df_to_correlate$overall_host_degree
df_to_correlate$y <- df_to_correlate$count_never_observed

correlation_hosts <- cor.test(df_to_correlate$x, df_to_correlate$y, use = "complete.obs", method = "pearson")
correlation_hosts

# Extract correlation coefficient and p-value
r_value <- round(correlation_hosts$estimate, 2)
p_value <- formatC(correlation_hosts$p.value, digits = 2)  # or round as you prefer
label_text_hosts <- paste0("r = ", r_value, ", p = ", p_value)
...
host_degree <- ggplot(df_to_correlate, aes(x = x, y = y)) +
  ...
  labs(
    ...
    title = paste("Hosts:", label_text_hosts)   # <--- add label in title
  ) +
  theme_minimal() + tme
...
# same for parasites
df_to_correlate_parasite <- never_parasite_degree_overall
df_to_correlate_parasite$x <- df_to_correlate_parasite$overall_parasite_degree
df_to_correlate_parasite$y <- df_to_correlate_parasite$count_never_observed

correlation_parasite <- cor.test(df_to_correlate_parasite$x, df_to_correlate_parasite$y, use = "complete.obs", method = "pearson")
correlation_parasite

# Extract correlation coefficient and p-value
r_value <- round(correlation_parasite$estimate, 2)
p_value <- formatC(correlation_parasite$p.value, digits = 2)  # or round as you prefer
label_text_parasites <- paste0("r = ", r_value, ", p = ", p_value)

parasite_degree <- ggplot(df_to_correlate_parasite, aes(x = x, y = y)) +
  geom_point(alpha = 0.6, size = 2, color = "rosybrown2") +
  geom_smooth(method = "lm", se = FALSE, color = "navy") +
  labs(
    x = "Overall degree",
    y = "Number of predicted, \nnon-observed interactions",
    title = paste("Parasites:", label_text_parasites)   # <--- add label in title
  ) +
  theme_minimal() + tme

# # Create the figure
# final_plot <- combine_plots(host_degree, parasite_degree) # Fig. 3c

# Your two plots (remove individual axis labels)
host_degree_clean <- host_degree +
  labs(x = NULL, y = NULL)

parasite_degree_clean <- parasite_degree +
  labs(x = NULL, y = NULL)

# Add bottom label with padding
final_plot <- plot_grid(
  # main plots
  plot_grid(host_degree_clean, parasite_degree_clean, ncol = 2, align = "hv"),
  # x label
...
df_summary <- df_summary %>% left_join(overall_parasite_degree, by="node_to")
df_summary <- df_summary %>% left_join(overall_host_degree, by="node_from")
...
# calculate proportion of years in which each interaction occurs, rather than year pairs
...
all((df_summary$avg_prop == 0) == (df_summary$avg_prop_year == 0)) # check
...
df_year_pair_diag <- df_year_pair_sep %>%
  filter(year1 == year2)

# only within-year data
df_summary_diag <- df_year_pair_diag %>%
  group_by(node_from, node_to) %>%
  summarise(
    avg_prop = mean(observed, na.rm = TRUE),       # proportion of years with observation
    avg_sigm_predicted = mean(year_pair_sigm_predicted, na.rm = TRUE),
    n_year_pairs = n(),  # number of year pairs contributing
    .groups = "drop"
  )
...
df_year_pair_offs <- df_year_pair_sep %>%
  filter(year1 != year2)

df_summary_offs <- df_year_pair_offs %>%
  group_by(node_from, node_to) %>%
  summarise(
    avg_prop = mean(observed, na.rm = TRUE),       # proportion of years with observation
    avg_sigm_predicted = mean(year_pair_sigm_predicted, na.rm = TRUE),
...
# order species by their degree
diff_df$node_from <- factor(diff_df$node_from, levels = host_order)
diff_df$node_to   <- factor(diff_df$node_to, levels = parasite_order)
...
#### ---- calculate time differences between years ----

# Create a time difference table for all layer pairs
# Use the layer_to_year mapping to get actual years
...
head(time_diff_table)

# Add time differences to result_summary (year names already added earlier)
...
#### ---- Fig. 4d: temporal distance correlation with evaluators ----

# Filter to only different years for temporal analysis
result_summary_temporal_dif <- result_summary_temporal %>% 
  filter(train_layer != test_layer)

# plot - using distance_km column name for compatibility with existing plotting functions
# Actually contains time_difference values
result_summary_temporal_dif <- result_summary_temporal_dif %>%
  mutate(distance_km = time_difference)  # rename for compatibility with plot function

cor_plot_dif_year_f05  <- make_cor_plot(result_summary_temporal_dif, evaluator = "f05_score", extra_theme = tme)

# pdf(
#   file   = "cor_plot_dif_year_f05.pdf",
#   width  = 4,
#   height = 4,
#   family = "Helvetica"
# )
# print(cor_plot_dif_year_f05)
# dev.off()     # close the file


# NNSE and temporal distance
cor_plot_dif_year_nnse  <- make_cor_plot(result_summary_temporal_dif, evaluator = "nnse", extra_theme = tme)
saveRDS(result_summary_temporal_dif, "results/hp_analysis/result_summary_temporal_dif.rds")

#### ---- MRM test: temporal scale ----
result_summary_temporal_dif <- readRDS("results/hp_analysis/result_summary_temporal_dif.rds")

# build square matrices of f0.5 and time difference
#    (layers must be in the same order for rows & cols)

# a) get a list of all unique layers
layers <- sort(unique(c(result_summary_temporal_dif$train_layer, result_summary_temporal_dif$test_layer)))

# b) initialize empty matrices
f05_mat      <- matrix(NA, nrow=length(layers), ncol=length(layers),
                      dimnames=list(layers, layers))
time_diff_mat <- f05_mat

# c) fill in each cell [i,j] with the corresponding f05_score and time_difference
for(i in layers) for(j in layers) {
  # subset rows where train=i and test=j
  sub <- result_summary_temporal_dif[result_summary_temporal_dif$train_layer==i & result_summary_temporal_dif$test_layer==j, ]
  if(nrow(sub)==1) {
    f05_mat[i,j]      <- sub$f05_score
    time_diff_mat[i,j] <- sub$time_difference
  }
}

# d) because MRM uses symmetric distance matrices, average [i,j] & [j,i]
f05_sym      <- sym_average(f05_mat)
time_diff_sym <- sym_average(time_diff_mat)

# save matrices for later use
write.csv(f05_sym, "results/hp_analysis/f05_distance_matrix.csv", row.names = TRUE)
write.csv(time_diff_sym, "results/hp_analysis/distance_matrix_km.csv", row.names = TRUE)


# e) convert to “dist” objects (lower triangle)
dist_f05      <- as.dist(f05_sym)
dist_km       <- as.dist(time_diff_sym)
dist_time     <- dist_km  # alias for temporal distance (used in MRM models)

# 4. run the MRM
#    — this will regress the F0.5‐distance matrix on the temporal–distance matrix
...
predictors <- c("dist_time", "c_connectance", "c_mat_size", "jaccard")
predictor_combinations <- unlist(lapply(1:length(predictors), function(n) {
  combn(predictors, n, simplify = FALSE)
}), recursive = FALSE)

# for each combination, add dist_time predictor, including only dist_time as a baseline
#predictor_combinations <- 
#  lapply(predictor_combinations, function(preds) {c("dist_time", preds)})
#predictor_combinations <- c(list("dist_time"), predictor_combinations) # add dist_time alone as a baseline
...
year_heatmap_f05 <- 
  ...
  geom_tile(color = "black", linewidth = 0.1) +
  # Then draw the diagonal tiles on top with black borders
  geom_tile(data = result_summary_temporal[result_summary_temporal$train_layer == result_summary_temporal$test_layer, ],
            color = "black", linewidth = 1.2) +  # Black borders only for diagonal tiles
...
print(year_heatmap_f05)

pdf(
  file   = "results/hp_analysis/paper_figs/year_heatmap_f05.pdf",
  width  = 6,    # inches
  height = 6,
  family = "Helvetica"   # or another installed font
)
...
### ---- additional stats ----
# For temporal data, we can look at specific year predictions if needed
# results_specific_year <- result_summary_temporal %>% filter(test_layer_name == "1985" & train_layer_name != "1985")
# mean(results_specific_year$f05_score)
# sd(results_specific_year$f05_score)

# is including external data better?
# run t-test via formula interface
wilcox_f05 <- wilcox.test(f05_score ~ layer_comparison,
                         data = result_summary_temporal,
                         exact = FALSE)  # turn off exact test for larger samples
wilcox_f05

# do we need welch/wilcoxon?
# first normality check
shapiro_f05 <- result_summary_temporal %>%
  group_by(layer_comparison) %>%
...
# For hosts (node_from)
host_occurrence <- df_occurrence %>%
  group_by(node_from) %>%
  summarise(num_years = n_distinct(test_layer)) %>%
  arrange(desc(num_years))

# For parasites (node_to)
parasite_occurrence <- df_occurrence %>%
  group_by(node_to) %>%
  summarise(num_years = n_distinct(test_layer)) %>%
  arrange(desc(num_years))

# View results
print(host_occurrence)
print(parasite_occurrence)

# join with degree data
# for hosts
host_data <- left_join(overall_host_degree, host_occurrence, by="node_from")

# For parasites
parasite_data <- left_join(overall_parasite_degree, parasite_occurrence, by="node_to")

# correlate
# calculate correlation
df_to_correlate_occurrence <- host_data
df_to_correlate_occurrence$x <- df_to_correlate_occurrence$overall_host_degree
df_to_correlate_occurrence$y <- df_to_correlate_occurrence$num_years

correlation_hosts_occ <- cor.test(df_to_correlate_occurrence$x, df_to_correlate_occurrence$y, use = "complete.obs", method = "pearson")
correlation_hosts_occ

# Extract correlation coefficient and p-value
r_value_occ <- round(correlation_hosts_occ$estimate, 2)
p_value_occ <- formatC(correlation_hosts_occ$p.value, digits = 2)  # or round as you prefer
label_text_hosts_occ <- paste0("r = ", r_value_occ, ", p = ", p_value_occ)

# plot occurrs

host_degree_occurrence <- ggplot(df_to_correlate_occurrence, aes(x = x, y = y)) +
  geom_point(alpha = 0.6, size = 2, color = "seagreen3") +
  geom_smooth(method = "lm", se = FALSE, color = "navy") +
  labs(
    x = "Overall degree",
    y = "Number of years occurs",
    title = paste("Hosts:", label_text_hosts_occ)   # <--- add label in title
  ) +
  theme_minimal() + tme

# same for parasites
df_to_correlate_parasite_occ <- parasite_data
df_to_correlate_parasite_occ$x <- df_to_correlate_parasite_occ$overall_parasite_degree
df_to_correlate_parasite_occ$y <- df_to_correlate_parasite_occ$num_years

correlation_parasite_occ <- cor.test(df_to_correlate_parasite_occ$x, df_to_correlate_parasite_occ$y, use = "complete.obs", method = "pearson")
correlation_parasite_occ

# Extract correlation coefficient and p-value
r_value_parasite <- round(correlation_parasite_occ$estimate, 2)
p_value_parasite <- formatC(correlation_parasite_occ$p.value, digits = 2)  # or round as you prefer
label_text_parasites_occ <- paste0("r = ", r_value_parasite, ", p = ", p_value_parasite)

parasite_degree_occurrence <- ggplot(df_to_correlate_parasite_occ, aes(x = x, y = y)) +
  geom_point(alpha = 0.6, size = 2, color = "rosybrown2") +
  geom_smooth(method = "lm", se = FALSE, color = "navy") +
  labs(
    x = "Overall degree",
    y = "Number of years occurs",
    title = paste("Parasites:", label_text_parasites_occ)   # <--- add label in title
  ) +
  theme_minimal() + tme

# # Create the final figure
# Your two plots (remove individual axis labels)
host_degree_occ_clean <- host_degree_occurrence +
  labs(x = NULL, y = NULL)

parasite_degree_occ_clean <- parasite_degree_occurrence +
  labs(x = NULL, y = NULL)

# Add bottom label with padding
final_plot_occ <- plot_grid(
  # main plots
  plot_grid(host_degree_occ_clean, parasite_degree_occ_clean, ncol = 2, align = "hv"),
  # x label
  ggdraw() + draw_label("Overall degree", fontface = "bold", size = 16),
  ncol = 1,
  rel_heights = c(1, 0.08)  # second element is space for x-axis label
...
# Add y label with padding
final_plot_occ <- plot_grid(
  ggdraw() + draw_label("Number of years present",
                        angle = 90, fontface = "bold", size = 16),
  final_plot_occ,
  ncol = 2,
  rel_widths = c(0.08, 1)   # first element is space for y-axis label
...
p_a_core <- make_facet_scatter_plot2(
  data = network_results_jaccard,
  evaluator = "f05_score",
  pivot_cols = "jaccard_parasites",
  facet_scales = "free_x",
  tme = tme
)
label_a <- paste0("Parasite overlap: ", rp_text("jaccard_parasites", "f05_score", network_results_jaccard))
p_a <- add_center_header(p_a_core, label_a, size = 12)

# (b) Hosts — keep y ticks; drop only the y-axis title (right column)
p_b_core <- make_facet_scatter_plot2(
  data = network_results_jaccard,
  evaluator = "f05_score",
  pivot_cols = "jaccard_hosts",
  facet_scales = "free_x",
  tme = tme
) + theme(axis.title.y = element_blank())
label_b <- paste0("Host overlap: ", rp_text("jaccard_hosts", "f05_score", network_results_jaccard))
...
label_c <- paste0("Edge overlap: ", rp_text("jaccard_edges", "f05_score", network_results_jaccard))
...
label_d <- paste0("Temporal distance: ", rp_text("distance_km", "f05_score", result_summary_temporal_dif))
...
year_jaccard_distance

pdf(
  file   = "results/hp_analysis/paper_figs/year_jaccard_distance.pdf",
  width  = 8,    # inches
  height = 8,
  family = "Helvetica"   # or another installed font
)
