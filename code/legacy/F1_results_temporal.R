...
# fig. 2c - heatmap red (year_heatmap_f1.pdf) ----
...
or_df_avg <- or_df %>% # 1) filter & prep
  ...
  ungroup() %>% # 3) average across layer combos and pivot long
  group_by(threshold) %>%
  summarise(across(
...
or_result_summary <- or_result_summary %>%
  mutate(layer_comparison = case_when(
    train_layer == test_layer ~ "Training year",
    train_layer != test_layer ~ "Test year"
  ))
...
for (layers_to_train in 1:num_layers) {
  for (layer_to_predict in 1:num_layers) {
    ...
    or_results <- rbind(or_results, data.frame(train_layer = layers_to_train,
...
or_result_summary_temporal <- or_result_summary

# Add time differences to main table
or_result_summary_temporal <- or_result_summary_temporal %>%
  left_join(time_diff_table %>% select(train_layer, test_layer, time_difference),
            by = c("train_layer", "test_layer")) %>%
  mutate(distance_km = time_difference)  # use time_difference for plotting compatibility

or_year_heatmap_f1 <- 
  ggplot(or_result_summary_temporal, aes(x = train_layer_name, y = test_layer_name, fill = f1_score)) +
  ...
  geom_tile(data = or_result_summary_temporal[or_result_summary_temporal$train_layer == or_result_summary_temporal$test_layer, ],
  ...
  labs(x = "Training year", y = "Test year", fill = "F1 score") +
  theme_minimal() +
  theme(
...
print(or_year_heatmap_f1)

pdf(
  file   = "results/hp_analysis/paper_figs/year_heatmap_f1_as_cutoff.pdf",
  width  = 6,    # inches
  height = 6,
...
table(or_df_year_pair$observed)
...
or_df_summary$node_from <- factor(or_df_summary$node_from, levels = host_order)
or_df_summary$node_to   <- factor(or_df_summary$node_to, levels = parasite_order)
...
pdf(
  file   = "results/hp_analysis/paper_figs/map_missing_links_f1_as_cutoff.pdf",
  width  = 11,    # inches
  height = 6,
  family = "Helvetica"   # or another installed font
)
