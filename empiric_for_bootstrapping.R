# ---- softImpute for predicting removed links in empirical networks for bootstraping ----
## ---- to do ----
#   CHECK IF COORDINATES IMPLY WHAT SHOULD BE INSTEAD OF NA - AND THEN WHAT IS THE MEANING OF REMOVING LINKS?
# Add bootstrapping for evaluating removed zeros
# Add what are the recalled zeros and ones, identity
# show in heatmap what were the removed zeros and ones
# fix bug! if we don't remove links we get far fewer links than we are supposed to <- fixed evaluation
# we get different results for different runs (same parameters)
# 3. check on cropped example
## optimize thresholding: by min-max, sigmoid (logistic regression). i don't think that min-max is approapriate because the values are coordinates. i used logistic regression instead. check wuth Shai
...
# add evaluation of removed links recall
df <- data.frame(
