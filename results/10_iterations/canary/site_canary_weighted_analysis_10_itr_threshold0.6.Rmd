---
title: "softimpute analysis pipeline - site scale (10 iterations)"
...
# Here we use `any(original_links != 0)` so that if the interaction is observed in at least one iteration, we count it.
df_island <- df %>%
...
df_never_observed <- df_summary %>%
  filter(avg_prop == 0, avg_sigm_predicted > 0.6) %>%
  group_by(node_from) %>%
...
df_all_itr_zero <- df %>%
  ...
  filter(removed == 1) %>% 
  filter(predicted_prob_sigm > 0.6) %>% 
  ungroup()
...
```
Conclusions: 
- increasing the threshold for classifying a link as an existing link from 0.5 to 0.6 lowered the recall and f1 (not dramatically) and increased specificity (and also precision, a little bit).
- there is now significant difference between aggregation scales. the evaluators that improved are higher for site scale and the ones the decreased are better for island scale.
- the effects of density of C, Jaccard (all aspects) and distance are now significant. 
- the missing significant interactions and variable importance have changed a bit consequently.
