...
df_removed <- df %>%
  ...
  mutate(predicted_bin_sigm = if_else(predicted_prob_sigm > 0.65, 1, 0)) %>% 
...
df_never_observed <- df_summary %>%
  filter(avg_prop == 0, avg_sigm_predicted > 0.65) %>%
  group_by(node_from) %>%
...
ggplot(df_summary, aes(x = node_to, y = node_from)) +
  ...
  geom_tile(
    data = df_summary %>% filter(avg_prop == 0, avg_sigm_predicted > 0.65),
...
df_all_itr_zero <- df %>%
  ...
  filter(removed == 1) %>% 
  filter(predicted_prob_sigm > 0.65) %>% 
  ungroup()
