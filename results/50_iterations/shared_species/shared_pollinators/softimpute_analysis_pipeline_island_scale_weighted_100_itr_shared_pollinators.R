...
df <- read_csv('weighted_scaled_island_net_60_100_itr_shared_pollinators.csv') # weighted, scaled
...
diff_df_filtered <- diff_df %>% filter(avg_sigm_predicted_diag < best_discrete_threshold & avg_sigm_predicted_offs >= best_discrete_threshold)
...
map_missing_links_diags_offs <- ggplot(df_plot, aes(x = node_to, y = node_from)) +
  ...
  scale_fill_manual(
    ...
    labels = c(
      "offs↑ only" = "Added location ≥ threshold\n& single location < 0.6",
      "diag↑ only" = "Single location ≥ threshold\n& added location < 0.6",
      "both↑"       = "Both ≥ threshold"
    )
  ) +
...
missing_links_shared_species <- predicted_links
missing_links <- read.csv("predicted_non_observed_links.csv", row.names = NULL)

# Add an identifier to each row for comparison
missing_links_shared_species$link_id <- paste(missing_links_shared_species$node_from,
                                              missing_links_shared_species$node_to,
                                              sep = "_")

missing_links$link_id <- paste(missing_links$node_from,
                               missing_links$node_to,
                               sep = "_")

# Find shared and unique links
shared_links <- intersect(missing_links_shared_species$link_id, missing_links$link_id)
unique_to_shared_species <- setdiff(missing_links_shared_species$link_id, missing_links$link_id)
unique_to_missing_links <- setdiff(missing_links$link_id, missing_links_shared_species$link_id)

# Percentage overlap
total_unique_links <- length(union(missing_links$link_id, missing_links_shared_species$link_id))
...
shared_df <- rbind(
  missing_links_shared_species %>% mutate(source = "shared_species"),
  missing_links %>% mutate(source = "missing_links")
)
...
shared_df <- shared_df %>%
  ...
  mutate(link_class = case_when(
    link_id %in% shared_links ~ "shared",
    link_id %in% unique_to_shared_species ~ "shared_species_only",
    link_id %in% unique_to_missing_links ~ "missing_links_only"
  ))
...
ggplot(shared_df, aes(x = node_to, y = node_from)) +
  ...
  scale_fill_manual(
    values = c(
      "shared" = "lightsteelblue",         # blue
      "shared_species_only" = "plum3",    # purple
      "missing_links_only" = "salmon1"      # orange
    ),
    name = "Link category",
    labels = c(
      "shared" = "Both tables",
      "shared_species_only" = "Shared species only",
      "missing_links_only" = "Missing links only"
    )
  ) +
  theme_minimal() +
  labs(x = "Pollinator", y = "Plant") +
...
# pie chart
link_summary <- tibble(
  category = c("Shared", "Shared species only", "Missing links only"),
...
ggplot(link_summary, aes(x = "", y = count, fill = category)) +
  ...
  scale_fill_manual(values = c(
    "Shared" = "lightsteelblue",        # blue
    "Shared species only" = "thistle",   # purple
    "Missing links only" = "salmon1"     # orange
  )) +
...
# result_site <- read_csv('canary_weighted_scaled_site_net_60_50_itr_overlapping_species.csv')
# 
