...
missing_links_shared_pollinators <- predicted_links
all_species_missing_links <- read.csv("predicted_non_observed_links.csv", row.names = NULL)

# Add an identifier to each row for comparison
missing_links_shared_pollinators$link_id <- paste(missing_links_shared_pollinators$node_from,
                                                  missing_links_shared_pollinators$node_to,
                                              sep = "_")

all_species_missing_links$link_id <- paste(all_species_missing_links$node_from,
                                           all_species_missing_links$node_to,
                               sep = "_")

# Find shared and unique links
shared_links <- intersect(missing_links_shared_pollinators$link_id, all_species_missing_links$link_id)
unique_to_shared_species <- setdiff(missing_links_shared_pollinators$link_id, all_species_missing_links$link_id)
unique_to_missing_links <- setdiff(all_species_missing_links$link_id, missing_links_shared_pollinators$link_id)

# Percentage overlap
total_unique_links <- length(union(all_species_missing_links$link_id, missing_links_shared_pollinators$link_id))
...
shared_df <- rbind(
  missing_links_shared_pollinators %>% mutate(source = "shared_pollinators"),
  all_species_missing_links %>% mutate(source = "all_species")
)
...
shared_df <- shared_df %>%
  ...
  mutate(link_class = case_when(
    link_id %in% shared_links ~ "shared_between_analyses",
    link_id %in% unique_to_shared_species ~ "shared_pollinators_only",
...
ggplot(shared_df, aes(x = node_to, y = node_from)) +
  ...
  scale_fill_manual(
    values = c(
      "shared_between_analyses" = "lightsteelblue",         # blue
      "shared_pollinators_only" = "plum3",    # purple
      "all_species_only" = "salmon1"      # orange
    ),
    name = "Link category",
    labels = c(
      "shared_between_analyses" = "Both analyses",
      "shared_pollinators_only" = "Shared pollinators only",
...
# pie chart
link_summary <- tibble(
  category = c("Both analyses", "Shared pollinators only", "All species only"),
...
ggplot(link_summary, aes(x = "", y = count, fill = category)) +
  ...
  scale_fill_manual(values = c(
    ...
    "Shared pollinators only" = "thistle",   # purple
