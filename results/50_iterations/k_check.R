...
## ---- data ----
df_50_site <- read_csv('canary_weighted_scaled_site_net_60_50_itr.csv')
df_50_site <- read_csv('k_check.csv')
...
# check if the removed interactions are different in the different interactions:

library(dplyr)

df_removed_1s <- df_removed %>% filter(original_binary == 1)
df_removed_0s <- df_removed %>% filter(original_binary == 0)

# Create a new column with the pair for easier comparison
df_pairs_1 <- df_removed_1s %>%
  mutate(pair = paste(node_from, node_to, sep = "_")) %>%
  select(itr, pair) %>%
  distinct()

# Now group by iteration and collect the pairs as a list
pair_lists <- df_pairs_1 %>%
  group_by(itr) %>%
  summarise(pairs = list(sort(unique(pair))))  # sort so order doesn't affect comparison

# Compare if all lists are the same
all_equal_1s <- all(sapply(pair_lists$pairs[-1], function(p) identical(p, pair_lists$pairs[[1]])))

print(all_equal_1s)

# now 0s
df_pairs_0 <- df_removed_0s %>%
  mutate(pair = paste(node_from, node_to, sep = "_")) %>%
  select(itr, pair) %>%
  distinct()

# Now group by iteration and collect the pairs as a list
pair_lists <- df_pairs_0 %>%
  group_by(itr) %>%
  summarise(pairs = list(sort(unique(pair))))  # sort so order doesn't affect comparison

# Compare if all lists are the same
all_equal_0s <- all(sapply(pair_lists$pairs[-1], function(p) identical(p, pair_lists$pairs[[1]])))
