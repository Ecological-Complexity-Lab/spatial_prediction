  ...
  for (layer_to_predict in 1:num_layers) {
    ...
    for (k in k_values) {
      for (lambda in lambda_values) {
        ...
        # Actual labels. add weighted values
    ...
    combined_results <- rbind(combined_results, data.frame(
      ...
      train_layer = layers_to_train,  # Layers to train
      test_layer = layer_to_predict,  # Layers to predict
      ...
      k = k,
      lambda = lambda,
      #species_high = node_from,
      #species_low = node_to, # correct these
      observed = original_links,
      reconstructed_value = predicted_values # correct this
    ))
  }
}
...
write.csv(combined_results, file = "combined_results_0.2_rem.csv", row.names = FALSE)
