...
# set.seed(456)
num_1_to_remove <- floor(sum(P, na.rm = T)*0.2)  # Number of links to remove
...
num_0_to_remove <- floor((nrow(P)*ncol(P)-sum(P, na.rm = T))*0.2)  # Number of links to remove
...
C[P_row_ids, P_col_ids] <- ifelse(is.na(C[P_row_ids, P_col_ids]), NA, C[P_row_ids, P_col_ids] + P)
