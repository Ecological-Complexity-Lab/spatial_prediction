...
df_thresh <- df_prepped %>%
  ...
  # 1) Compute metrics within each iteration
  group_by(emln_id, train_layer, test_layer, itr, threshold) %>%
  summarise(
    TP = sum(original_binary == 1 & predicted_bin == 1, na.rm = TRUE),
    FN = sum(original_binary == 1 & predicted_bin == 0, na.rm = TRUE),
    TN = sum(original_binary == 0 & predicted_bin == 0, na.rm = TRUE),
    FP = sum(original_binary == 0 & predicted_bin == 1, na.rm = TRUE),
    ...
    mcc = if_else((TP+FP) > 0 & (TP+FN) > 0 & (TN+FP) > 0 & (TN+FN) > 0,
                  (TP * TN - FP * FN) /
  ...
  # 2) Average metrics across iterations (macro-averaging)
  ...
  summarise(
    ...
    iterations        = n(),  # number of bootstrap iterations used
    .groups = "drop"
  )
...
# 1) Per-iteration metrics (one row per emln_id/train_layer/test_layer/itr)
iter_metrics <- df_removed %>%
...
# 2) Macro-averaged metrics across iterations (this is what you asked for)
result_summary <- iter_metrics %>%
  group_by(emln_id, train_layer, test_layer) %>%
  summarise(
    # average the METRICS (not the counts)
    specificity       = mean(specificity,       na.rm = TRUE),
    precision         = mean(precision,         na.rm = TRUE),
    recall            = mean(recall,            na.rm = TRUE),
    f1_score          = mean(f1_score,          na.rm = TRUE),
    ...
    iterations = n(),  # how many bootstrap iterations contributed
    .groups = "drop"
  )
