    ...
    for (i in 1:n_sim) {
      ...
      for (k in k_values) {
        for (lambda in lambda_values) {
          ...
          # For the row species: these come from the left singular vectors (fit$u)
          latent_traits_rows <- fit$u
          # Make sure to assign species names from the matrix C
          rownames(latent_traits_rows) <- rownames(C)
          latent_traits_rows_df <- as.data.frame(latent_traits_rows)
          # Rename the latent trait columns for clarity (trait_1, trait_2, etc.)
          n_traits <- ncol(latent_traits_rows_df)
          colnames(latent_traits_rows_df)[1:n_traits] <- paste0("trait_", seq_len(n_traits))
          # Add the species name and a role identifier
          latent_traits_rows_df$species <- rownames(C)
          latent_traits_rows_df$role <- "row"
          
          # For the column species: these come from the right singular vectors (fit$v)
          latent_traits_cols <- fit$v
          rownames(latent_traits_cols) <- colnames(C)
          latent_traits_cols_df <- as.data.frame(latent_traits_cols)
          n_traits_cols <- ncol(latent_traits_cols_df)
          colnames(latent_traits_cols_df)[1:n_traits_cols] <- paste0("trait_", seq_len(n_traits_cols))
          latent_traits_cols_df$species <- colnames(C)
          latent_traits_cols_df$role <- "column"
          
          # --- Add metadata to both data frames ---
          # You can add metadata columns to record the iteration and layer info, as well as k and lambda
          latent_traits_rows_df$itr <- i
          latent_traits_rows_df$train_layer <- layers_to_train
          latent_traits_rows_df$test_layer <- layer_to_predict
          latent_traits_rows_df$k <- k
          latent_traits_rows_df$lambda <- lambda_values
          
          latent_traits_cols_df$itr <- i
          latent_traits_cols_df$train_layer <- layers_to_train
          latent_traits_cols_df$test_layer <- layer_to_predict
          latent_traits_cols_df$k <- k
          latent_traits_cols_df$lambda <- lambda_values
          
          # Combine the two data frames (so each row indicates one species, along with its latent trait vector)
          latent_traits_combined <- rbind(latent_traits_rows_df, latent_traits_cols_df)
          
          # Append this result to your master data frame
          all_latent_traits <- rbind(all_latent_traits, latent_traits_combined)
