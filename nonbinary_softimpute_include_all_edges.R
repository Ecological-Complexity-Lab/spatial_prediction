  ...
  for (layer_to_predict in 1:num_layers) {
    ...
    for (k in k_values) {
      for (lambda in lambda_values) {
        ...
        removed_edges <- data.frame(
          ...
          predicted_values = predicted_values,
          stringsAsFactors = FALSE
        ) # check if row is here, if not, combine as it is (delete from all_edges)
        
        # After computing removed_edges
        if (nrow(removed_edges) == 0) {
          # No edges were removed
          all_edges$was_removed <- 0
          all_edges$predicted_values <- NA
        } else {
          # Edges were removed, so merge them
          all_edges <- merge(
            all_edges,
            removed_edges,
          ...
          # For edges that weren't removed, was_removed will be NA. Set them to 0:
          all_edges$was_removed[is.na(all_edges$was_removed)] <- 0
          # predicted_values remains NA where was_removed = 0
        }
    ...
    combined_results <- rbind(
      ...
      data.frame(
        ...
        k = k,
        lambda = lambda,
        node_to = all_edges$node_to,
        node_from = all_edges$node_from,
        observed = all_edges$original_links,
        was_removed = all_edges$was_removed,
        reconstructed_value = all_edges$predicted_values
      )
    )
...
write.csv(combined_results, file = "combined_results_0.2_rem_values_nonbinary.csv", row.names = FALSE)
